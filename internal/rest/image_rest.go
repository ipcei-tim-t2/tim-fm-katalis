...
func handleFileProblemDetails(log logr.Logger, code int, p *opgmodels.ProblemDetails) {
	log.Info(">>> [Image Response with error", "error", code, "details", p)
}
...
func (r *ImageReconciler) CreateImage(ctx context.Context, image *v1beta1.Image, feder *v1beta1.Federation) error {
	log := log.FromContext(ctx)
	log.Info(">>> [Image][REST] Creating external image via REST API")
	imageId, err := uuid.Parse(image.Spec.ImageId)
	if err != nil {
		return err
	}
	fileReqBody := opgmodels.UploadFileMultipartBody{
		...
		ImgInsSetArch:   opgmodels.CPUArchType(image.Spec.ImageBody.ImgInsSetArch),
	...
	if err != nil {
		log.Error(err, ">>> [Image[REST] Error serializing multipart body")
		return err
	}
	res, err := r.GetOPGClient(
		...
		body)
	if err != nil {
		log.Error(err, ">>> [Image][REST] Error creating image via REST API")
	...
	case statusCode >= 200 && statusCode < 300:
		log.Info(">>> [Image][REST] Status code 2xx received from OPG API", "status", statusCode)
	...
	case statusCode == 400:
		...
		image.Status.State = v1beta1.ImageStateError
		log.Info(">>> [Image][REST] Couldn't be created", "Detail", res.ApplicationproblemJSON400.Detail)
	...
	upErr := r.Status().Update(ctx, image)
	if upErr != nil {
		log.Error(upErr, ">>> [Image][REST] Error updating image status")
	}
	return nil
}
...
func (r *ImageReconciler) UpdateImageStatus(ctx context.Context, image *v1beta1.Image, fed *v1beta1.Federation) error {
	...
	if image.Spec.ImageNotifLink == "" {
		log.Info(">>> [Image][REST] No callback StatusLink configured in Federation, skipping App callback")
		return nil
	}
	log.Info(">>> [Image][REST] Sending App callback to Guest", "callback", image.Spec.ImageNotifLink, "state", image.Status.State)
	imageId, err := uuid.Parse(image.Spec.ImageId)
	if err != nil {
		return err
	}
	...
	if err != nil {
		log.Error(err, ">>> [Image][REST] Error sending App callback")
		return err
	}
	statusCode := res.StatusCode()
	switch {
	case statusCode >= 200 && statusCode < 300:
		log.Info(">>> [Image][REST] Successfully sent Image callback to Guest", "status", statusCode)
	case statusCode == 400:
		handleFileProblemDetails(log, statusCode, res.ApplicationproblemJSON400)
		image.Status.State = v1beta1.ImageStateError
	case statusCode == 401:
...
func (r *ImageReconciler) DeleteImage(ctx context.Context, image *v1beta1.Image, fed *v1beta1.Federation) error {
	log := log.FromContext(ctx)
	log.Info(">>> [Image][REST] Deleting external image")
	// we should delete the image from the federation partner via REST API
	imageId, err := uuid.Parse(image.Spec.ImageId)
	if err != nil {
		return err
	}
	...
	if err != nil {
		log.Error(err, ">>> [Image][REST] Error deleting external image")
		return err
	}
	...
	switch {
	case statusCode >= 200 && statusCode < 300:
		log.Info(">>> [Image][REST] Deleted external image successfully")
	...
	upErr := r.Status().Update(ctx, image.DeepCopy())
	if upErr != nil {
		log.Error(upErr, errorUpdatingImageStatusMsg)
		return upErr
	}
	return nil
}
